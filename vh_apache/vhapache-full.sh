...
opcaohttps()
	{
    	read -p "Deseja opção HTTPS? S/N: " OPHTTPS;
    		...
    		elif [ "$OPHTTPS" = 'N' -o "$OPHTTPS" = 'n' ]; then 
        		VOPHTTPS1="";
            	VOPHTTPS2="";
...
opcaosenha()
    ...
    read -p "Deseja opção Senha no Site? S/N: " OPSENHA;
    		...
    		elif [ "$OPSENHA" = 'N' -o "$OPSENHA" = 'n' ]; then 
        			VOPSENHA="";
...
opcaoredict()
    ...
    read -p "Deseja opção Redirecionamento (Forçar HTTPS)? S/N: " OPREDIRECT;
    		...
    		elif [ "$OPREDIRECT" = 'N' -o "$OPREDIRECT" = 'n' ]; then 
        		VREDIRECT="";
                OPREDIRECT="DESABILITADO";
    		else 
    			echo "[ERRO] OPÇAO INCORRETA";
...
echo -e "\n[PROCESSO] CRIANDO VIRTUAL HOST... ";
	if [ -f "/etc/apache2/sites-available/$NOMESITE" ]; then
